package aii.steps;

import org.junit.Test;

import aii.utils.APIConstants;
import aii.utils.APIGlobalVariables;
import io.cucumber.java.en.When;
import io.restassured.RestAssured;
import io.restassured.http.ContentType;

public class API_Model_Regression {

//Validation of User Story-5965
	@Test
	@When("I enter required body information for required scenarios <US5965> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us5965_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1-1

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 1-2

		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2
		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 0,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3
		String payload4 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 550000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 130000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload4).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4

		String payload5 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 450000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 0,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload5).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario5

		String payload6 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 500000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 120000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload6).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-5965");
	}

//Validation of US-6287 Rules Coverage A Minimum Limit Change
	@Test
	@When("I enter required body information for required scenarios <US6287> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us6287_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 120000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 70000,\r\n"
				+ "    \"covE\": 70000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario2
		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario3

		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"ONWER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4

		String payload4 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload4).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-6287");
	}

//Validation of US-6760 Claim Reserves- Owner Occupied Endorsement
	@Test
	@When("I enter required body information for required scenarios <US6760> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us6760_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2

		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3

		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-6760");

	}

//Validation of User Story-6625 Update Loss Assessment
	@Test
	@When("I enter required body information for required scenarios <US6625> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us6625_and_i_validate_response_body_status_code_is_ok() {

		// Scenario1
		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Jerald\",\r\n"
				+ "      \"middle\": \"API\",\r\n" + "      \"last\": \"Bilavandran\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2

		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Jerald\",\r\n"
				+ "      \"middle\": \"API\",\r\n" + "      \"last\": \"Bilavandran\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3

		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Jerald\",\r\n"
				+ "      \"middle\": \"API\",\r\n" + "      \"last\": \"Bilavandran\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-6625");

	}

//Validation of User Story-6288 Modify Eligibility for Prior Non-Weather Related Losses
	@Test
	@When("I enter required body information for required scenarios <US6288> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us6288_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Jerald\",\r\n"
				+ "      \"middle\": \"API\",\r\n" + "      \"last\": \"Bilavandran\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 400000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2

		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 500000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3

		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-6288");

	}

//Creating application for different types of products HO3-HO4-DP1-DP3
	@Test
	@When("I enter required body information for required scenarios for <HO3-HO4-DP1-DP3> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_for_ho3_ho4_dp1_dp3_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1 HO4

		String payload1 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO4\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "    \"underwritingQuestions\":{\r\n"
				+ "    \"businessActivityOnPremises\":false,\r\n" + "    \"childOrAdultDaycareOnPremises\":false,\r\n"
				+ "    \"existingDamage\":false,\r\n" + "    \"exoticAnimalsOnPremises\":false,\r\n"
				+ "    \"foundationDisclosure\":false,\r\n" + "    \"haveKnownSinkholeActivity\":false,\r\n"
				+ "    \"havePreviousInsuranceLapse\":false,\r\n" + "    \"havePriorInsurance\":true,\r\n"
				+ "    \"locationCurrentlyVacantOrUnoccupied\":false,\r\n" + "    \"propertyOccupied\":false,\r\n"
				+ "    \"locationInSpecialFloodHazardArea\":false,\r\n" + "    \"ownOrCareForAnimals\":false,\r\n"
				+ "    \"ownRecreationalVehicles\":false,\r\n" + "    \"previousConviction\":false,\r\n"
				+ "    \"previousForeclosure\":false,\r\n" + "    \"previousSinkholeInspection\":false,\r\n"
				+ "    \"previousSinkholeLoss\":false,\r\n"
				+ "    \"previouslyInsuredWithAmericanIntegrity\":false,\r\n"
				+ "    \"assignmentOfBenefits\":false,\r\n" + "    \"firstPartyLawsuit\":false,\r\n"
				+ "    \"priorAIPolicyNumber\":\"XXXXXXX\",\r\n" + "    \"priorFloodLoss\":false,\r\n"
				+ "    \"priorInsuranceCancelled\":false,\r\n" + "    \"priorLosses\":false,\r\n"
				+ "    \"priorNonWeatherLosses\":false,\r\n" + "    \"nonWeatherWaterLosses\":false,\r\n"
				+ "    \"shortSaleOrForeclosure\":false,\r\n" + "    \"swimmingPoolHotTubSpa\":false,\r\n"
				+ "    \"enclosed\":false,\r\n" + "    \"haveExcessiveLiabilityExposure\":false,\r\n"
				+ "    \"unrelatedRentersOrBoarders\":true\r\n" + "},\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 25000,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": true,\r\n" + "    \"waterDamageLimited\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-04-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "    \"runInsuranceScore\": true\r\n" + "  } ,\r\n" + "\"workflowFlags\": {\r\n"
				+ "      \"bindApplication\": true\r\n" + "} \r\n" + "},\r\n" + "  \"requestId\": \"8720-88776\",\r\n"
				+ "  \"responseDetail\": true\r\n" + "}\r\n" + "";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload1)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2 HO3

		String payload2 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "    \"underwritingQuestions\":{\r\n"
				+ "    \"businessActivityOnPremises\":false,\r\n" + "    \"childOrAdultDaycareOnPremises\":false,\r\n"
				+ "    \"existingDamage\":false,\r\n" + "    \"exoticAnimalsOnPremises\":false,\r\n"
				+ "    \"foundationDisclosure\":false,\r\n" + "    \"haveKnownSinkholeActivity\":false,\r\n"
				+ "    \"havePreviousInsuranceLapse\":false,\r\n" + "    \"havePriorInsurance\":true,\r\n"
				+ "    \"locationCurrentlyVacantOrUnoccupied\":false,\r\n" + "    \"propertyOccupied\":false,\r\n"
				+ "    \"locationInSpecialFloodHazardArea\":false,\r\n" + "    \"ownOrCareForAnimals\":false,\r\n"
				+ "    \"ownRecreationalVehicles\":false,\r\n" + "    \"previousConviction\":false,\r\n"
				+ "    \"previousForeclosure\":false,\r\n" + "    \"previousSinkholeInspection\":false,\r\n"
				+ "    \"previousSinkholeLoss\":false,\r\n"
				+ "    \"previouslyInsuredWithAmericanIntegrity\":false,\r\n"
				+ "    \"assignmentOfBenefits\":false,\r\n" + "    \"firstPartyLawsuit\":false,\r\n"
				+ "    \"priorAIPolicyNumber\":\"XXXXXXX\",\r\n" + "    \"priorFloodLoss\":false,\r\n"
				+ "    \"priorInsuranceCancelled\":false,\r\n" + "    \"priorLosses\":true,\r\n"
				+ "    \"priorNonWeatherLosses\":true,\r\n" + "    \"nonWeatherWaterLosses\":false,\r\n"
				+ "    \"shortSaleOrForeclosure\":false,\r\n" + "    \"swimmingPoolHotTubSpa\":false,\r\n"
				+ "    \"enclosed\":false,\r\n" + "    \"haveExcessiveLiabilityExposure\":false,\r\n"
				+ "    \"unrelatedRentersOrBoarders\":true\r\n" + "},\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 25000,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": true,\r\n" + "    \"waterDamageLimited\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-04-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "    \"runInsuranceScore\": true\r\n" + "  } ,\r\n" + "\"workflowFlags\": {\r\n"
				+ "      \"bindApplication\": true\r\n" + "} \r\n" + "},\r\n" + "  \"requestId\": \"8720-88776\",\r\n"
				+ "  \"responseDetail\": true\r\n" + "}\r\n" + "";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload2)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3 DP3

		String payload3 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-04-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "    \"underwritingQuestions\":{\r\n"
				+ "    \"businessActivityOnPremises\":false,\r\n" + "    \"childOrAdultDaycareOnPremises\":false,\r\n"
				+ "    \"existingDamage\":false,\r\n" + "    \"exoticAnimalsOnPremises\":false,\r\n"
				+ "    \"foundationDisclosure\":false,\r\n" + "    \"haveKnownSinkholeActivity\":false,\r\n"
				+ "    \"havePreviousInsuranceLapse\":false,\r\n" + "    \"havePriorInsurance\":true,\r\n"
				+ "    \"locationCurrentlyVacantOrUnoccupied\":false,\r\n" + "    \"propertyOccupied\":false,\r\n"
				+ "    \"locationInSpecialFloodHazardArea\":false,\r\n" + "    \"ownOrCareForAnimals\":false,\r\n"
				+ "    \"ownRecreationalVehicles\":false,\r\n" + "    \"previousConviction\":false,\r\n"
				+ "    \"previousForeclosure\":false,\r\n" + "    \"previousSinkholeInspection\":false,\r\n"
				+ "    \"previousSinkholeLoss\":false,\r\n"
				+ "    \"previouslyInsuredWithAmericanIntegrity\":false,\r\n"
				+ "    \"assignmentOfBenefits\":false,\r\n" + "    \"firstPartyLawsuit\":false,\r\n"
				+ "    \"priorAIPolicyNumber\":\"XXXXXXX\",\r\n" + "    \"priorFloodLoss\":true,\r\n"
				+ "    \"priorInsuranceCancelled\":false,\r\n" + "    \"priorLosses\":false,\r\n"
				+ "    \"priorNonWeatherLosses\":false,\r\n" + "    \"nonWeatherWaterLosses\":true,\r\n"
				+ "    \"shortSaleOrForeclosure\":false,\r\n" + "    \"swimmingPoolHotTubSpa\":false,\r\n"
				+ "    \"enclosed\":false,\r\n" + "    \"haveExcessiveLiabilityExposure\":false,\r\n"
				+ "    \"unrelatedRentersOrBoarders\":true\r\n" + "},\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 25000,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": true,\r\n" + "    \"waterDamageLimited\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "    \"runInsuranceScore\": true\r\n" + "  } ,\r\n" + "\"workflowFlags\": {\r\n"
				+ "      \"bindApplication\": true\r\n" + "} \r\n" + "},\r\n" + "  \"requestId\": \"8720-88776\",\r\n"
				+ "  \"responseDetail\": true\r\n" + "}\r\n" + "";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload3)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4 DP1

		String payload4 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP1\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-04-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "    \"underwritingQuestions\":{\r\n"
				+ "    \"businessActivityOnPremises\":false,\r\n" + "    \"childOrAdultDaycareOnPremises\":false,\r\n"
				+ "    \"existingDamage\":false,\r\n" + "    \"exoticAnimalsOnPremises\":false,\r\n"
				+ "    \"foundationDisclosure\":false,\r\n" + "    \"haveKnownSinkholeActivity\":false,\r\n"
				+ "    \"havePreviousInsuranceLapse\":false,\r\n" + "    \"havePriorInsurance\":true,\r\n"
				+ "    \"locationCurrentlyVacantOrUnoccupied\":false,\r\n" + "    \"propertyOccupied\":false,\r\n"
				+ "    \"locationInSpecialFloodHazardArea\":false,\r\n" + "    \"ownOrCareForAnimals\":false,\r\n"
				+ "    \"ownRecreationalVehicles\":false,\r\n" + "    \"previousConviction\":false,\r\n"
				+ "    \"previousForeclosure\":false,\r\n" + "    \"previousSinkholeInspection\":false,\r\n"
				+ "    \"previousSinkholeLoss\":false,\r\n"
				+ "    \"previouslyInsuredWithAmericanIntegrity\":false,\r\n"
				+ "    \"assignmentOfBenefits\":false,\r\n" + "    \"firstPartyLawsuit\":false,\r\n"
				+ "    \"priorAIPolicyNumber\":\"XXXXXXX\",\r\n" + "    \"priorFloodLoss\":true,\r\n"
				+ "    \"priorInsuranceCancelled\":false,\r\n" + "    \"priorLosses\":false,\r\n"
				+ "    \"priorNonWeatherLosses\":false,\r\n" + "    \"nonWeatherWaterLosses\":true,\r\n"
				+ "    \"shortSaleOrForeclosure\":false,\r\n" + "    \"swimmingPoolHotTubSpa\":false,\r\n"
				+ "    \"enclosed\":false,\r\n" + "    \"haveExcessiveLiabilityExposure\":false,\r\n"
				+ "    \"unrelatedRentersOrBoarders\":true\r\n" + "},\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 450000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 90000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 100000,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 25000,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": true\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 50000,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": true,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": true,\r\n" + "    \"waterDamageLimited\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "    \"runInsuranceScore\": true\r\n" + "  } ,\r\n" + "\"workflowFlags\": {\r\n"
				+ "      \"bindApplication\": true\r\n" + "} \r\n" + "},\r\n" + "  \"requestId\": \"8720-88776\",\r\n"
				+ "  \"responseDetail\": true\r\n" + "}\r\n" + "";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload4)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("Applications have been created for different types of products HO3-HO4-DP1-DP3");
	}

//Validation of User Story-6300 Home Computer
	@Test
	@When("I enter required body information for required scenarios <US6300> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us6300_and_i_validate_response_body_status_code_is_ok() {

		// Scenario1

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2

		String payload2 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 0,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload2).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3

		String payload3 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload3).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4

		String payload4 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload4).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 5

		String payload5 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG6718A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-02-01\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better Than Average (750-849)\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"TENANT\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"INTEGRITYSELECT\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 100000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"lossAssessment\": 0,\r\n" + "    \"fungiMoldEtc\": \"10000/10000\",\r\n"
				+ "    \"ordinanceOrLaw\": 0,\r\n" + "    \"serviceLine\": \"None\",\r\n"
				+ "    \"waterBackupSumpOverflow\": \"\",\r\n" + "    \"refrigeratedPersonalProperty\": false\r\n"
				+ "  },\r\n" + "  \"optionalCoverages\": {\r\n" + "    \"animalLiability\": true,\r\n"
				+ "    \"animalLiabilityAmount\": 0,\r\n" + "    \"carportPoolCageScreen\": \"\",\r\n"
				+ "    \"increasedDwellingReplacementCost\": true,\r\n" + "    \"limitedTheft\": false,\r\n"
				+ "    \"personalInjury\": false,\r\n" + "    \"personalPropertyReplacementCost\": false,\r\n"
				+ "    \"sinkholeLoss\": \"10\",\r\n" + "    \"waterDamageExclusion\": false,\r\n"
				+ "    \"waterDamageLimited\": false,\r\n" + "    \"unitOwnersCovA\": false,\r\n"
				+ "    \"unitOwnersCovC\": false,\r\n" + "    \"storageSlipRental\": false\r\n" + "  },\r\n"
				+ "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-02-01\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload5).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-6300");
	}

//Validation of User Story-5512 HO3 App Roof Material
	@Test
	@When("I enter required body information for required scenarios <US5512> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us5512_and_i_validate_response_body_status_code_is_ok() {

		// Scenario HO3 RC1 App Roof Material= 3 Tab Composition Shingle

		String payload1 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-12-18\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \" \"\r\n" + "  },\r\n"
				+ "  \"reservePackage\": \"SILVER\",\r\n" + "  \"deductibles\": {\r\n" + "    \"aop\": 2500,\r\n"
				+ "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n" + "  },\r\n"
				+ "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"NONE\",\r\n" + "    \"fireAlarmCredit\": \"NONE\",\r\n"
				+ "    \"securedCommunityBldg\": \"NONE\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"leakProtection\": \"NONE\",\r\n" + "    \"leakManufacturer\": \"NONE\",\r\n"
				+ "    \"leakManufacturerModelNumber\": \"NONE\"\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n" + "    \"roofShape\": \"GABLE\",\r\n"
				+ "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-04-22\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload1)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-5512");

	}

//Validation of User Story-8583 UI Arbitration 
	@Test
	@When("I enter required body information for required scenarios <US8583> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us8583_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1 Basic_MMA_Yes

		String payload1 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload1)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2 Basic_MMA_Blank

		String payload2 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"SILVER\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload2)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3 Diamond_MMA_No

		String payload3 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"DIAMOND\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":false\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload3)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4 Gold_MMA_Yes

		String payload4 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"GOLD\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload4)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 5 Silver_MMA_No

		String payload5 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"SILVER\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":false\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload5)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-8853");
	}

//Validation of User Story-7119 Animal Liability
	@Test
	@When("I enter required body information for required scenarios <US7119> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us7119_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1 Animal_Basic_MMA_Yes

		String payload1 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 300000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"49000\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload1)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 2 Animal_Basic_MMA_Blank

		String payload2 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 400000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload2)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 3 Animal_Diamond_MMA_No

		String payload3 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"DIAMOND\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 500000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"499999\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":false\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload3)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 4 Animal_Gold_MMA_Yes

		String payload4 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"GOLD\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 400000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 299999,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"299999\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":true\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload4)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		// Scenario 5 Animal_Silver_MMA_No

		String payload5 = "{\r\n" + "  \"customerId\": \"HIPPO\",\r\n" + "  \"customerKey\": \"\",\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"HO3\",\r\n" + "  \"producer\": \"AG1777A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-05-06\",\r\n"
				+ "    \"carrier\": \"AVATAR\"\r\n" + "  },\r\n" + "  \"entityType\": \"INDIVIDUAL\",\r\n"
				+ "  \"insured\": {\r\n" + "    \"name\": {\r\n" + "      \"commercial\": \"Jake Ward\",\r\n"
				+ "      \"first\": \"Jake\",\r\n" + "      \"last\": \"Ward\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1970-05-20\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"111-111-1111\",\r\n" + "      \"name\": \"MOBILE\"\r\n" + "    },\r\n"
				+ "    \"email\": \"abc@abc.com\"\r\n" + "  },\r\n"
				+ "  \"insuranceScore\": \"Better than Average (750-849)\",\r\n" + "  \"insuredProperty\": {\r\n"
				+ "    \"dwellingType\": \"SINGLE FAMILY\",\r\n" + "    \"location\": {\r\n"
				+ "       \"houseNumber\":\"41\",\r\n" + "      \"address\": \"Kelly Ridge Rd\",\r\n"
				+ "      \"city\": \"Frostproof\",\r\n" + "      \"county\": \"Polk County\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33843\"\r\n" + "    },\r\n"
				+ "    \"constructionType\": \"MASONRY\",\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"numberOfStories\": 2,\r\n" + "    \"bceg\": 1\r\n" + "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 5,\r\n"
				+ "    \"squareFeet\": 2409,\r\n" + "    \"yearOfConstruction\": 2014,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2014,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"SILVER\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 2500,\r\n" + "    \"hurricane\": 2,\r\n" + "    \"windHailExclusion\": false\r\n"
				+ "  },\r\n" + "  \"coverages\": {\r\n" + "    \"covA\": 300000,\r\n" + "    \"covB\": 10,\r\n"
				+ "    \"covC\": 50,\r\n" + "    \"covE\": 300000,\r\n" + "    \"covF\": 5000,\r\n"
				+ "    \"homeSystemsProtection\": \"NONE\",\r\n" + "    \"identityRecovery\": \"NONE\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"lossAssessment\": 1000,\r\n"
				+ "    \"ordinanceOrLaw\": 25,\r\n" + "    \"waterBackupSumpOverflow\": \"NONE\"\r\n" + "  },\r\n"
				+ "  \"optionalCoverages\": {\r\n" + "    \"animalLiabilityAmount\":\"300001\",\r\n"
				+ "    \"carportPoolCageScreen\": \"10000\",\r\n" + "    \"personalPropertyReplacementCost\": true,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false,\r\n" + "    \"mandatoryMediationArbitration\":false\r\n"
				+ "  },\r\n" + "  \"discounts\": {\r\n" + "    \"burglarAlarmCredit\": \"NONE\",\r\n"
				+ "    \"fireAlarmCredit\": \"NONE\",\r\n" + "    \"securedCommunityBldg\": \"NONE\",\r\n"
				+ "    \"sprinklerSystem\": \"NONE\",\r\n" + "    \"leakProtection\": \"NONE\",\r\n"
				+ "    \"leakManufacturer\": \"NONE\",\r\n" + "    \"leakManufacturerModelNumber\": \"NONE\"\r\n"
				+ "  },\r\n" + "  \"windMitigation\": {\r\n" + "    \"openingProtection\": \"HURRICANE IMPACT\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"secondaryWaterResistance\": true\r\n" + "  },\r\n"
				+ "  \"existingTransactionNumber\": \"\",\r\n" + "  \"effectiveDate\": \"2022-05-06\",\r\n"
				+ "  \"source\": \"\",\r\n" + "  \"dataReports\": {\r\n" + "    \"runInsuranceScore\": false\r\n"
				+ "  },\r\n" + "  \"workflowFlags\": {\r\n" + "    \"bindApplication\": false\r\n" + "  }\r\n"
				+ "} ,\r\n" + "  \"requestId\": \"8720-787\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive()
				.basic(APIGlobalVariables.hippoUsername, APIGlobalVariables.hippoPassword).body(payload5)
				.contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

		System.out.println("All scenarios have passed for US-7119");
	}

//Validation of User Story-8830 DP3 Owner Cov-C Default
	@Test
	@When("I enter required body information for required scenarios <US8830> and I validate response body status code is ok")
	public void i_enter_required_body_information_for_required_scenarios_us8830_and_i_validate_response_body_status_code_is_ok() {

		// Scenario 1 Basic Cov-C as 12k

		String payload1 = "{\r\n" + "  \"customerId\": \"AI\",\r\n" + "  \r\n" + "  \"customerKey\": null,\r\n"
				+ "  \"quoteRequest\": {\r\n" + "  \"productType\": \"DP3\",\r\n" + "  \"producer\": \"AG1730A1\",\r\n"
				+ "  \"priorPolicy\": {\r\n" + "    \"expirationDate\": \"2022-09-05\",\r\n"
				+ "    \"premium\": 1200,\r\n" + "    \"carrier\": \"OTHER\",\r\n"
				+ "    \"originalEffectiveDate\": \"\",\r\n" + "    \"priorPolicyNumber\": \"\"\r\n" + "  },\r\n"
				+ "  \"entityType\": \"JOINT\",\r\n" + "  \"insured\": {\r\n" + "    \"name\": {\r\n"
				+ "      \"commercial\": \"Victor Hedman and Sanna Hedman\",\r\n" + "      \"first\": \"Victor\",\r\n"
				+ "      \"middle\": \"\",\r\n" + "      \"last\": \"Hedman\"\r\n" + "    },\r\n"
				+ "    \"dateOfBirth\": \"1990-12-18\",\r\n" + "    \"primaryPhone\": {\r\n"
				+ "      \"value\": \"(904)371-4627\",\r\n" + "      \"name\": \"Victor Hedman\"\r\n" + "    },\r\n"
				+ "    \"email\": \"\"\r\n" + "  },\r\n" + "  \"insuranceScore\": \"\",\r\n" + "  \"coInsured\": {\r\n"
				+ "    \"firstName\": \"Sanna\",\r\n" + "    \"middleName\": \"\",\r\n"
				+ "    \"lastName\": \"Hedman\",\r\n" + "    \"dateOfBirth\": \"1991-09-06\"\r\n" + "  },\r\n"
				+ "  \"insuredProperty\": {\r\n" + "    \"dwellingType\": \"SINGLE FAMILY\",\r\n"
				+ "      \"location\": {\r\n" + "      \"address\": \"1710 E Kirby\",\r\n"
				+ "      \"city\": \"TAMPA\",\r\n" + "      \"county\": \"Hillsborough\",\r\n"
				+ "      \"state\": \"FL\",\r\n" + "      \"zipCode\": \"33604\"\r\n" + "    },\r\n"
				+ "    \"bceg\": \"UNGRADED\",\r\n" + "    \"constructionType\": \"FRAME\",\r\n"
				+ "    \"numberOfUnits\": 1,\r\n" + "    \"occupancy\": \"OWNER\",\r\n"
				+ "    \"monthsOccupied\": 12,\r\n" + "    \"numberOfStories\": 1,\r\n"
				+ "    \"storyUnitIsLocated\": 0,\r\n" + "    \"distanceToCoast\": \"15 mi to less than 20 mi\"\r\n"
				+ "  },\r\n" + "  \"dwelling\": {\r\n"
				+ "    \"distanceToHydrant\": \"LESS THAN OR EQUAL 1000 FEET\",\r\n" + "    \"protectionClass\": 3,\r\n"
				+ "    \"squareFeet\": 1068,\r\n" + "    \"yearOfConstruction\": 2018,\r\n"
				+ "    \"yearRoofMaterialUpdated\": 2018,\r\n" + "    \"propertyManaged\": false,\r\n"
				+ "    \"shortTermRental\": false,\r\n" + "    \"roofMaterial\": \"Concrete/Clay Tile\"\r\n"
				+ "  },\r\n" + "  \"reservePackage\": \"BASIC\",\r\n" + "  \"deductibles\": {\r\n"
				+ "    \"aop\": 1000,\r\n" + "    \"hurricane\": 10,\r\n" + "    \"windHail\": 1000,\r\n"
				+ "    \"windHailExclusion\": false\r\n" + "  },\r\n" + "  \"coverages\": {\r\n"
				+ "    \"covA\": 350000,\r\n" + "    \"covB\": 2,\r\n" + "    \"covC\": 12000,\r\n"
				+ "    \"covE\": 100000,\r\n" + "    \"covF\": 5000,\r\n" + "    \"covL\": 0,\r\n"
				+ "    \"covM\": 0,\r\n" + "    \"homeComputer\": 2500,\r\n"
				+ "    \"homeSystemsProtection\": \"50000/10000\",\r\n" + "    \"identityRecovery\": \"15000\",\r\n"
				+ "    \"fungiMoldEtc\": \"10000/10000\",\r\n" + "    \"ordinanceOrLaw\": 0,\r\n"
				+ "    \"serviceLine\": \"None\",\r\n" + "    \"waterBackupSumpOverflow\": \"\",\r\n"
				+ "    \"refrigeratedPersonalProperty\": false\r\n" + "  },\r\n" + "  \"optionalCoverages\": {\r\n"
				+ "    \"animalLiability\": true,\r\n" + "    \"animalLiabilityAmount\": 0,\r\n"
				+ "    \"carportPoolCageScreen\": \"\",\r\n" + "    \"increasedDwellingReplacementCost\": true,\r\n"
				+ "    \"limitedTheft\": false,\r\n" + "    \"personalInjury\": false,\r\n"
				+ "    \"personalPropertyReplacementCost\": false,\r\n" + "    \"sinkholeLoss\": \"10\",\r\n"
				+ "    \"waterDamageExclusion\": false,\r\n" + "    \"waterDamageLimited\": false,\r\n"
				+ "    \"unitOwnersCovA\": false,\r\n" + "    \"unitOwnersCovC\": false,\r\n"
				+ "    \"storageSlipRental\": false\r\n" + "  },\r\n" + "  \"discounts\": {\r\n"
				+ "    \"burglarAlarmCredit\": \"CENTRAL STATION\",\r\n"
				+ "    \"fireAlarmCredit\": \"CENTRAL STATION\",\r\n" + "    \"proofOfUpdates\": \"ROOF ONLY\",\r\n"
				+ "    \"securedCommunityBldg\": \"\",\r\n" + "    \"sprinklerSystem\": \"NONE\",\r\n"
				+ "    \"openFoundation\": false,\r\n" + "    \"hardiplankSiding\": false,\r\n"
				+ "    \"military\": true\r\n" + "  },\r\n" + "  \"windMitigation\": {\r\n"
				+ "    \"openingProtection\": \"NONE\",\r\n" + "    \"roofCover\": \"FBC EQUIVALENT\",\r\n"
				+ "    \"roofDeck\": \"\",\r\n" + "    \"roofDeckAttachment\": \"\",\r\n"
				+ "    \"roofShape\": \"GABLE\",\r\n" + "    \"roofToWallAttachment\": \"\",\r\n"
				+ "    \"secondaryWaterResistance\": false\r\n" + "  },\r\n"
				+ "  \"effectiveDate\": \"2022-09-05\",\r\n" + "   \"dataReports\": {\r\n"
				+ "  	\"runInsuranceScore\": false\r\n" + "  }	\r\n" + "} ,\r\n"
				+ "  \"requestId\": \"8720-88776\",\r\n" + "  \"responseDetail\": true\r\n" + "}";

		RestAssured.baseURI = APIConstants.BASE_URI;

		RestAssured.given().auth().preemptive().basic(APIGlobalVariables.harvUsername, APIGlobalVariables.harvPassword)
				.body(payload1).contentType(ContentType.JSON).post().prettyPeek().then().assertThat().statusCode(200);

	}
}
