...
public class TC16741_DP1_ValidateErrorMessagesonQuoteLeveUICharacteristics extends CommonMethods {
	...
	public void user_creates_new_quote_again_tc16741() {
		...
		click(quote.btnCopyToMailAddress);
		click(quote.btnCopyToBillAddress);
		selectDropdownText(driver.findElement(By.id("CustomerPhonePrimary.PhoneName")), ConfigsReader.getProperty("phonetype"));
		sendText(driver.findElement(By.id("CustomerPhonePrimary.PhoneNumber")), ConfigsReader.getProperty("phonenumber"));
		wait(1);
		click(quote.btnSaveAndQuote);
		wait(2);
	}
...
//		wait(3);
	...
	@When("User sets prior carrier as new purchase removes expiration date sets occupancy as tenant occupied annual and validates issue messages")
	public void user_sets_prior_carrier_as_new_purchase_removes_expiration_date_sets_occupancy_as_tenant_occupied_annual_and_validates_expected_messages()
			throws Exception {
		clickOnAnyLink(driver, "Policy");
		verify_AnyText_IsVisible(driver, "Acknowledgment of Consent to Rate must be provided within 15 days of issuance");		
		selectDropdownText(policyChevron.ddPreviousCarrier, "New Purchase");
		selectDropdownText(policyChevron.ddOccupancy, "Tenant Occupied");
		selectDropdownText(policyChevron.ddMonthsOccupied, "Annual");
		wait(1);
...
//				"Owner Occupied risks that are occupied 0 to 3 months per year are ineligible for coverage under the DP1 policy form");
	...
	public void user_checks_Wind_Hail_exlusion_and_validates_issue_messages()
		...
		actions.moveToElement(driver.findElement(By.id("Building.WindHailExcludedInd"))).click().build().perform();
		Thread.sleep(10000);
