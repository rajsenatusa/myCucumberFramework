...
@regression @mtr404 @dp1regression @dp1regressionFEB
Feature: TC 16870--DP1 - UWMNGR - Company Cancellation - Reason Property Ineligible - Reinstatement form_AIIC RI 11 14
  ...
  Scenario: Validate Policy is Cancelled & on reinstatement correct Reinstatement form is attaching to Policy File
    ...
    And User enters all required information on policy information screen <mtr404>
    And User enters DP1 product selection information and current date as effective date
    And User enters Producer Code <mtr404>
    And User enters all required information on DP1 quote screen with current date as prior policy date <mtr404>
		And User enters all required information on DP1 dwelling screen <mtr404>
